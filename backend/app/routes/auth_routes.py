...
from ..model.user import Token,  UserCreate,  UserResponse, UserRequest, LoginRequest, EmailModel, ForgotPasswordRequest, ResetPasswordRequest, PasswordResetResponse
...
async def register(user: UserCreate):
    ...
    user_data = {"username": user.username,
                 "email": user.email, "hashed_password": hashed_pw, "isVerified": False}
    ...
    return {"id": user_data["id"], "username": user_data["username"], "email": user_data["email"], "isVerified": user_data["isVerified"], "message": "Account Created"}
...
async def check_verification_status(email: str):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Error checking verification status")
...
async def forgot_password(request: ForgotPasswordRequest):
    ...
    try:
        ...
        reset_token = create_url_safe_token({"email": request.email, "type": "password_reset"})
